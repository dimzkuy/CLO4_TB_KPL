...
// Tampilkan gambar film jika ada
const filmImageUrl = localStorage.getItem("selectedFilmImage");
if (filmImageUrl) {
  document.getElementById("filmImage").src = filmImageUrl;
} else {
  document.getElementById("filmImage").style.display = "none";
}
...
function downloadPDF() {
  ...
  doc.setFontSize(10);
  doc.text(
    "Terima kasih telah memesan tiket! Tunjukkan tiket ini saat masuk bioskop.",
    105,
    135,
    { align: "center" }
  );
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.text("CinemaBook - Your Premium Cinema Experience", 105, 145, {
    align: "center",
  });
...
function downloadCSV() {
  ...
  const csvData = [
    ...
    `"${film}","${formattedDate}","${jamTayang}","${kursi.join(
      " | "
    )}","Rp ${totalHarga.toLocaleString("id-ID")}"`,
  ].join("\n");
